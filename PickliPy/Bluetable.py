...
Key Bluetable assumptions (simplified vs Assay):

* Uses **one source plate**.
  - The SRC inventory must contain exactly one unique Plate barcode.
  - That barcode is still written into Echo picklist rows and Plate:Works
    inventory/process files.
  - The code does *not* implement any multi-source-plate mechanisms.

* The SRC inventory may contain the same "Compound Name" multiple times with
  **different stock concentrations**.
  - Bluetable selects the source well by matching the concentration specified
    in the blue-table file.

* Destination layouts come from multiple XLSX files in ``folderdst``.
  - Each XLSX (first worksheet) defines one destination plate.
  - Destination barcode is taken from the XLSX filename stem.
  - All dispenses are merged into one Echo picklist.

Outputs
-------

All outputs are written into an ``outs`` folder created (or reused) inside the
destination folder:

  ``<folderdst>/outs``

This mirrors the typical Mathematica workflow where outputs live next to the
input blue-tables.
"""
...
from typing import Any, DefaultDict, Dict, List, Mapping, MutableMapping, Optional, Sequence, Tuple
...
class SrcRow:
    """One SRC inventory row after normalization.

    Note: barcode is stored once (in SrcInventory). The per-row key is the
    source well name.
    """
...
class SrcInventory:
    ...
    rows: List[SrcRow]
    min_volume_ul: float
    remaining_ul: MutableMapping[str, float]  # keyed by src_well
    src_name: Mapping[str, str]  # src_well -> compound_label
    vehicles: Mapping[str, str]  # src_well -> vehicle type
    compound_to_sources: Mapping[str, Tuple[List[str], float]]  # compound_label -> ([wells], stock_base)
    is_ldv: bool
...
def _fmt_float(x: Any, *, max_decimals: int = 6) -> str:
    ...
    return f"{v2:.{max_decimals}f}".rstrip("0").rstrip(".")
...
def _cell_table_str(rows: Sequence[Sequence[Any]]) -> str:
    ...
    return "\n".join(f"{a:<{w0}}  {b:>{w1}}".rstrip() for a, b in rows2)
...
def print_plate_grid(title: str, cells: Sequence[Sequence[str]]) -> None:
    ...
    # Split into lines.
    cell_lines: List[List[List[str]]] = [[(c or "-").splitlines() for c in row] for row in cells]
    ...
    for r in range(PLATE_ROWS_96):
        ...
        for k in range(h):
            line_cells = [cell_lines[r][c][k] if k < len(cell_lines[r][c]) else "" for c in range(PLATE_COLS_96)]
...
def _detect_minvol_and_columns(src_matrix: List[List[Any]]) -> Tuple[int, int, Optional[int]]:
    ...
    * bluetable-style: minvol in H1, vehicle in G, 'new' marker in H
    * assay-style:     minvol in G1, no vehicle column, 'new' marker in G
    """
...
def _export_src_like_workbook(
    ...
    *,
    ...
    """Write an SRC-like workbook, updating only the volume column."""

    vol_by_well: Dict[str, float] = {as_str(rr["src_well"]).strip(): float(rr["volume_ul"]) for rr in raw_rows_current}
    ...
    for i in range(1, len(mat)):
        ...
        if w in vol_by_well:
            while len(row) <= volume_col_idx0:
                row.append("")
            row[volume_col_idx0] = vol_by_well[w]
...
def _load_src_inventory(inventory_xlsx: Path, folderdst: Path, *, out_dir: Path) -> SrcInventory:
    ...
    # Enforce a single source-plate barcode.
    barcodes = sorted({rr["barcode"] for rr in raw_rows if rr["barcode"] != ""})
    if len(barcodes) != 1:
        raise PicklyPyConfigError(
            "Error: Bluetable requires exactly one source plate barcode in the inventory SRC worksheet. "
            f"Found {len(barcodes)}: {barcodes}"
        )
    source_plate_barcode = barcodes[0]

    # Update volumes from survey XMLs found in folderdst.
    survey = load_survey_volumes(folderdst)
    survey_vols = dict(survey.volumes_ul)

    # Map UnknownBarCode -> the single barcode.
    if any(bc == "UnknownBarCode" for (bc, _well) in survey_vols.keys()):
        remapped: Dict[Tuple[str, str], float] = {}
        for (bc, well), vol in survey_vols.items():
            remapped[(source_plate_barcode if bc == "UnknownBarCode" else bc, well)] = vol
        survey_vols = remapped

    if survey_vols:
        missing: List[str] = []
        for rr in raw_rows:
            k = (source_plate_barcode, rr["src_well"])
            if k in survey_vols:
                rr["volume_ul"] = float(survey_vols[k])
            else:
                # WL behavior: set to 0 when survey exists but value is missing.
                missing.append(f"{source_plate_barcode}_{rr['src_well']}")
                rr["volume_ul"] = 0.0
        ...
        # Restore volumes for wells marked 'new' (WL bluetable behavior).
        orig_vol_by_well: Dict[str, float] = {}
        for r in src_matrix[1:]:
            if len(r) < 5:
                continue
            w = as_str(r[0]).strip()
            ...
            if v is None:
                continue
            orig_vol_by_well[w] = float(v)
        for rr in raw_rows:
            if rr["is_new"]:
                w = rr["src_well"]
                if w in orig_vol_by_well:
                    rr["volume_ul"] = orig_vol_by_well[w]
    ...
    # Validations
    # 1) Source wells must be unique within the (single) source plate.
    seen: set[str] = set()
    dups: List[str] = []
    for rr in raw_rows:
        w = rr["src_well"]
    ...
    if dups:
        raise PicklyPyConfigError(
            "Error: duplicate well assignment in SRC worksheet within the source plate: "
            + ", ".join(sorted(set(dups)))
        )

    # NOTE: Bluetable intentionally allows the same Compound Name at multiple
    # stock concentrations. Do NOT enforce the assay-style restriction.

    # Normalize rows: convert stock to base units and append to compound name.
    rows: List[SrcRow] = []
    for rr in raw_rows:
        try:
            ...
            raise PicklyPyConfigError(
                f"Unknown unit {rr['unit_raw']!r} in SRC row for {source_plate_barcode}_{rr['src_well']} ({rr['compound']})."
            )
        stock_base = float(integer_chop(float(rr["stock_raw"]) * f))
    ...
    remaining_ul: Dict[str, float] = OrderedDict((r.src_well, r.volume_ul) for r in rows)
    src_name: Dict[str, str] = {r.src_well: r.compound_label for r in rows}
    vehicles: Dict[str, str] = {r.src_well: r.vehicle for r in rows}

    # Pool alternative wells for identical compounds.
    ...
    for comp, comp_rows in tmp.items():
        comp_rows_sorted = sorted(comp_rows, key=lambda x: x.src_well)
        compound_to_sources[comp] = ([r.src_well for r in comp_rows_sorted], comp_rows_sorted[0].stock_base)
...
def _calc_echo_volume_nl(
    *,
    ...
    """WL dilute: Round[concentrated*1000*wellvolume*final/stock, 2.5]."""
    if stock_base <= 0:
    ...
    # Keep the "no silent zero" safeguard from the assay lineage.
...
def _choose_src_well(*, src_candidates: Sequence[str], vol_nl: float, inv: SrcInventory) -> str:
    need_ul = float(vol_nl) / 1000.0
    for w in src_candidates:
        have_ul = float(inv.remaining_ul.get(w, 0.0))
        if have_ul >= float(inv.min_volume_ul) + need_ul:
            inv.remaining_ul[w] = have_ul - need_ul
            return w
...
def _manual_dilute(
    *,
    ...
    """WL manualdilute: parse trailing digits as stock concentration."""
...
def _write_notebook_tables_xlsx(path: Path, echo_cells: Sequence[Sequence[str]], manual_cells: Sequence[Sequence[str]]) -> None:
    """Write the per-destination notebook tables workbook (Echo + Manual)."""
    wb = openpyxl.Workbook()
    wb.remove(wb.active)
...
def _process_one_bluetable_file(
    ...
    *,
    ...
    manual_table_raw: List[List[List[List[Any]]]] = [[[] for _ in range(PLATE_COLS_96)] for _ in range(PLATE_ROWS_96)]
    ...
    for block_idx, cols0 in enumerate(BLUETABLE_COLS_0BASED):
        ...
        cecho: List[Tuple[str, List[str], float, float]] = []
        cmanual: List[Tuple[str, str, float]] = []
        ...
        if cmanual:
            ...
            for dst_well, name_label, final_base in cmanual:
                name, vol_stock, vol_medium = _manual_dilute(
        ...
        # Wells with Echo but no manual: fill "Medium".
        ...
        vehicle_defs: List[Tuple[str, List[str], float]] = []
        ...
        if vehicle_defs:
            ...
            for vtype, v_src_candidates, req_nl in vehicle_defs:
                for dst_well in sorted(wells_with_vehicle.get(vtype, set()), key=parse_well_name):
                    have_nl = per_vehicle_per_well.get((vtype, dst_well), 0.0)
                    if have_nl + 1e-9 < req_nl:
                        l0.append((dst_well, list(v_src_candidates), float(req_nl - have_nl)))
    ...
    for dst_well, src_candidates, vol_nl in all_events:
        by_compound[tuple(src_candidates)][dst_well] += float(vol_nl)
    ...
    chosen: List[Tuple[str, str, float]] = []  # dst, src_well, vol_nl
    for dst_well, src_candidates, vol_nl in ordered_events:
        src_well = _choose_src_well(src_candidates=src_candidates, vol_nl=vol_nl, inv=inv)
        chosen.append((dst_well, src_well, float(vol_nl)))

    by_src: "OrderedDict[str, List[Tuple[float, str]]]" = OrderedDict()
    for dst_well, src_well, vol_nl in chosen:
        if inv.is_ldv and vol_nl > 500.0:
            remaining = vol_nl
            while remaining > 0:
                chunk = min(500.0, remaining)
                by_src.setdefault(src_well, []).append((float(chunk), dst_well))
                remaining -= chunk
        else:
            by_src.setdefault(src_well, []).append((float(vol_nl), dst_well))

    print("Volumes used:")
    for src_well, pairs in by_src.items():
        used_ul = sum(v for v, _ in pairs) / 1000.0
        comp = inv.src_name.get(src_well, src_well)
        print(f"  {comp}   {inv.source_plate_barcode}_{src_well}   {used_ul} uL")
        total_use_rows.append((comp, src_well, float(used_ul)))

    for src_well, pairs in by_src.items():
        row: List[Any] = [inv.source_plate_barcode, src_well, dst_barcode]
        for vol_nl, dst_well in pairs:
            row.append(integer_chop(vol_nl))
            row.append(dst_well)
        picklist_rows.append(row)

    # Build/print manual and Echo tables (Mathematica Grid replacement).
    ...
    echo_raw: List[List[List[List[Any]]]] = [[[] for _ in range(PLATE_COLS_96)] for _ in range(PLATE_ROWS_96)]
    for dst_well, src_well, vol_nl in chosen:
        i, j = _grid_index_96(dst_well)
        echo_raw[i][j].append([inv.src_name.get(src_well, src_well), integer_chop(vol_nl)])
    ...
    out_tables = outs_dir / f"{dst_barcode}_notebook_tables.xlsx"
...
def run_bluetable(*, picklist_name: str, inventory_xlsx: Path, folderdst: Path) -> RunResult:
    """Run the Bluetable generator.

    All outputs are written to ``folderdst/outs``.
    """

    if not folderdst.exists() or not folderdst.is_dir():
        raise PicklyPyConfigError(f"Destination folder not found: {folderdst}")

    outs_dir = folderdst / "outs"
    ...
    print("Python translation of bluetable.wls (PickliPy.Bluetable)")
    print(f"Outputs folder: {outs_dir}")

    inv = _load_src_inventory(inventory_xlsx, folderdst, out_dir=outs_dir)
    ...
    picklist_csv_path = outs_dir / f"{picklist_name}_Picklist.csv"
    picklist_text_lines = [",".join(as_str(x) for x in row) for row in picklist_rows]
    ...
    for comp, src_well, used_ul in total_use_rows:
        by_src[src_well].append((comp, float(used_ul)))
    for src_well, items in by_src.items():
        comp = items[0][0]
        total_ul = sum(u for _, u in items)
        remains = inv.remaining_ul.get(src_well, 0.0)
        print(f"{comp}\t\t{inv.source_plate_barcode}_{src_well}\t{total_ul}\tremains (ul): {remains}")
    ...
    for rr in src_matrix0[1:]:
        ...
        if w == "":
            continue
        export_rows.append({"src_well": w, "volume_ul": float(inv.remaining_ul.get(w, as_number(rr[4]) or 0.0))})

    after_dispense_path = outs_dir / f"after_dispense_{inventory_xlsx.name}"
    ...
    inventory_rows.append(make_inventory_row(inv.source_plate_barcode, rack=1, pos=1))
    ...
    inventory_path = outs_dir / f"{picklist_name}_Inventory.csv"
    write_text(inventory_path, inventory_csv)
    print(inventory_csv.rstrip("\n"))
    print(f"Saving: {inventory_path}")

    print("***** Process_SRC")
    process_src = inv.source_plate_barcode + "\n"
    process_src_path = outs_dir / f"{picklist_name}_Process_SRC.txt"
    ...
    process_dst_path = outs_dir / f"{picklist_name}_Process_DST.txt"
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="PicklyPy.Bluetable",
        description="Blue-table picklist generator (PickliPy.Bluetable).",
    )
    p.add_argument(
        "picklist_name",
        help="Picklist base name (no extension). Output files use this prefix.",
    )
    p.add_argument(
        "inventory_xlsx",
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    picklist_name = Path(args.picklist_name).stem
    inventory_xlsx = Path(args.inventory_xlsx)
    folderdst = Path(args.folderdst)

    def _run() -> None:
        run_bluetable(picklist_name=picklist_name, inventory_xlsx=inventory_xlsx, folderdst=folderdst)
