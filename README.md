...
See blank design files in the project folder above, or create ones from prompt (see Agentic Skill below).
...
### Usage of files generated by PickliPy
- **Picklist (csv)** - This follows Beckman Echo API formatting. Use it in Revvity Plate::Works ECHO Pick command
- **Inventory files (csv)** - Use it in Revvity Plate::Works Entry/RACK or INCUBATOR configuration
- **Process files (txt)** - Use it in Revvity Plate::Works Entry/RACK or INCUBATOR configuration
- **merge table.txt** - Use it with "Merge File" in DataWarrior while working with analysis results. Use it in Image Analyst MKII, Multi-dimensional Open dialog, Position Names tab, “Load position names from well-plate-compound list” command (magic wand button).
- **compound and concentration map files** - Use them in Revvity Harmony with Copy/Paste to create layouts.  
...
Clone this git and open folder in Visual Studio Code with OpenAI Codex extension installed. Needs Python (3.10+) and requirements.txt installations (see below).
