...
def parse_printmap(xml_file, lib_file):
    ...
    # Load library file and match compounds (if provided)
    if lib_file:
        try:
            ...
            # If library matching fails, continue without compounds
            print(f"Warning: Could not match compounds: {e}")
            merged = df.copy()
   ...
   # Add barcode columns
    ...
    # Reorder columns
    # Build column list - include Compound only if library was provided
...
class PickliPyQC(tk.Tk):
    ...
    def _process_files(self):
        ...
        if lib_file and not os.path.exists(lib_file):
            messagebox.showerror("Error", f"Library file not found: {lib_file}")
            return
        # If lib_file is empty string, set to None for cleaner logic
        if not lib_file:
            lib_file = None
    ...
    def _generate_rerun_picklist(self, parsed_results, output_dir):
        ...
        for (src_barcode, src_well, dst_barcode), transfers in grouped.items():
            ...
            for t in transfers:
                line_parts.append(str(t['volume'] * 1000))  # Convert to nL
