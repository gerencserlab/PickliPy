...
Version 2.0 - Enhanced UI with log box, checkbox file selection, 
              Assay/Screening mode with smart compound lookup
...
def get_column_case_insensitive(df, target_name):
    ...
    for col in df.columns:
        # Skip non-string columns (NaN, numbers, etc.)
...
def build_compound_lookup(picklist_file, picklist_mode, log_func=None):
    ...
    if not picklist_file or not os.path.exists(picklist_file):
        ...
        return {}, {}, 2.5  # Default dead volume
    ...
    well_alternatives = defaultdict(list)  # (barcode, compound) -> [wells]
    dead_volume_ul = 2.5  # Default
    all_barcodes_for_expansion = set()  # For * wildcard expansion
    ...
    if src_sheet_name:
        ...
        if all([barcode_col, well_col, compound_col]):
            ...
            for _, row in src_df.iterrows():
                ...
                # Handle * wildcard - expand to all known barcodes
                if barcode == '*':
...
def find_matching_surveys(log_dir, print_files, log_func=None):
    ...
    # Collect all survey files with their barcodes and timestamps
    surveys = []
    ...
    # Match each print file to its survey
    matches = {}
    for print_file in print_files:
        # Get print file's source barcode and timestamp
        try:
        ...
        # Find surveys with matching barcode that are BEFORE the print timestamp
        candidates = [
            s for s in surveys
        ...
        if candidates:
            # Get the most recent one (closest to print time)
...
def parse_printmap(xml_file, compound_lookup=None):
    ...
    # Build column list - include Compound only if lookup was provided
...
class PickliPyQC(tk.Tk):
    def __init__(self):
        ...
        self.xml_files = []
        self.file_check_vars = []  # List of BooleanVars for checkboxes
    ...
    def _create_widgets(self):
        ...
        # === ACTION BUTTONS (before log so they're always visible) ===
    ...
    def _process_files(self):
        ...
        # Load compound lookup if not already loaded
        ...
        # For smart rerun: collect print file paths and find matching surveys
        selected_print_files = []
        if mode == 'print':
        ...
        for idx in selected_indices:
            ...
            try:
                ...
                # Add matched survey info for print mode
                if mode == 'print':
        ...
        if parsed_results:
            ...
            # Generate smart rerun picklist if there are skipped transfers
            if mode == 'print':
    ...
    def _generate_smart_rerun_picklist(self, parsed_results, output_dir):
        ...
        for result in parsed_results:
            ...
            # Load survey volumes for alternative wells
            survey_volumes = {}
            if matched_survey:
                survey_volumes = load_survey_volumes(matched_survey)
            
            # Collect post-dispense volumes from print data
            print_volumes = {}
        ...
        # Get dead volume from plate type lookup (use first result's plate type)
        dead_volume_ul = self.dead_volume_ul
        
        # Process each skipped transfer
        rerun_lines = []
        excluded_count = 0
        switched_count = 0
        ...
        for t in skipped_transfers:
            ...
            dst_well = t['dst_well']
            volume_nl = t['volume_ul'] * 1000.0  # Convert to nL
            compound = t['compound']
            
            # Find best well to use
            chosen_well = original_well
            needed_ul = dead_volume_ul + t['volume_ul']
            
            # Check if we have compound lookup and alternatives
            if compound and compound != 'NA' and self.well_alternatives:
                ...
                for candidate in alternatives:
                    # Get volume: prefer print (post-dispense) for used wells, survey for unused
                    ...
                    # No well with sufficient volume
        ...
        # Group by (src_barcode, src_well, dst_barcode) for Echo format
